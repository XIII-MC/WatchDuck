...
import watchduck.check.checks.combat.TpAura.TpAuraA;
import watchduck.check.checks.movement.speed.SpeedA;
import watchduck.exempt.Exempt;
import watchduck.utils.BlockUtils;
import watchduck.utils.BoundingBox;
import watchduck.utils.PastLocation;
import watchduck.utils.SampleList;
...
public class PlayerData {
        ...
        if(player != null) {
            ...
            for (Player p : Bukkit.getOnlinePlayers()) {
                ...
                if ((data.alertstoggled && !d) || (d && !p.getName().equals(player.getName()))) { // TODO: /alerts command test later && (!d && !player.getName().equals(p.getName()))
                    TextComponent Flag = new TextComponent("§3§lWatchDuck §7»§f " + name + " §7failed §f" + check.name + " §7(§3x" + getFlags(name, check.name) + "§7)");
            ...
            if(WatchDuck.instance.configUtils.getBooleanFromConfig("config", "testMode")) {
                TextComponent Flag = new TextComponent("§3§lWatchDuck §7»§f " + name + " §7failed §f" + check.name + " §7(§bx" + getFlags(name, check.name) + "§7)");
                Flag.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(text).create()));
    ...
    public void ban(String r, boolean ban, boolean kick, String punishMessage, boolean broadcast) {
        if(WatchDuck.instance.configUtils.getBooleanFromConfig("config", "testMode")) {
            player.sendTitle("§cYou would be kicked by now.", "§3&lWatchDuck §7»§f " + punishMessage);
        } else {
            if(broadcast && ban || kick) {
                Bukkit.broadcastMessage(WatchDuck.instance.configUtils.getConvertedStringFromConfig("checks", player, ".Messages.broadcastMessage"));
            }
            if(ban) {
                ...
                Bukkit.getScheduler().runTask(WatchDuck.instance, () -> {
                    Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "ban " + player.getName() + " " + WatchDuck.instance.configUtils.getConvertedStringFromConfig("checks", player, ".Messages.banMessage"));
                });
            } else {
                if(kick) {
                    Bukkit.getScheduler().runTask(WatchDuck.instance, () -> {
                        Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "kick " + player.getName() + " " + WatchDuck.instance.configUtils.getConvertedStringFromConfig("checks", player, ".Messages.kickMessage"));
                    });
                }
            }
        }
